...
class Edge:
    ...
    # Находится ли точка строго вне окружности
    def is_not_inside_circle(self, point):
    ...
    def draw(self, tk):
        ...
        for e in self.edges:
            ...
            for s in e.gaps:
               ...
               -self.alpha).ry(-self.beta).rz(-self.gamma) * (1/self.c)) and \
                  e.is_not_inside_circle(e.r3(1).rz(
                  -self.alpha).ry(-self.beta).rz(-self.gamma) * (1/self.c)):
